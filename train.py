...
def select_action(state): # [N,C,H,W]
    global steps_done
    eps_threshold = EPS_END + (EPS_START - EPS_END) * math.exp(-1. * steps_done / EPS_DECAY)
    steps_done += state.shape[0]
    if random.random() > eps_threshold:
        with torch.no_grad():
            return policy_net(state).max(1).indices # max(1) -> max in dimension 1
    else:
...
env = Env(Robot(),Object(),vis=VIS,gamma=GAMMA)
...
for i_episode  in range(NUM_EPISODES):
    ...
    for t in count():
        ...
        next_state = update_state(state, obs) if not terminated else None
        memory.push(state, action, next_state, reward) # [N,C,H,W] [N] [N,C,H,W] [N] state, action and reward need N dimension for   torch.cat(batch.state). before 'for i' loop N=V, now N=1
        # 1.3 update state
        state = next_state
